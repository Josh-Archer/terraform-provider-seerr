...
type DiscoverSliderDataSourceItemModel struct {
...
type DiscoverSliderDataSourceModel struct {
	...
	Sliders []DiscoverSliderDataSourceItemModel `tfsdk:"sliders"`
}

func NewDiscoverSliderDataSource() datasource.DataSource { return &DiscoverSliderDataSource{} }
...
func (d *DiscoverSliderDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Read discovery sliders from Seerr via `/api/v1/settings/discover`.",
				...
				NestedObject: schema.NestedAttributeObject{
					Attributes: map[string]schema.Attribute{
						...
						"type": schema.Int64Attribute{
							Computed:            true,
							MarkdownDescription: "The type of the slider.",
						},
						...
						"title": schema.StringAttribute{
							Computed:            true,
							MarkdownDescription: "The title of the slider.",
						},
						"data": schema.StringAttribute{
							Computed:            true,
							MarkdownDescription: "The data/query for the slider.",
						},
					},
				},
			},
...
func (d *DiscoverSliderDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	data.Sliders = make([]DiscoverSliderDataSourceItemModel, 0, len(apiSliders))
	for _, s := range apiSliders {
		item := DiscoverSliderDataSourceItemModel{
			ID:      d.toInt64(s["id"]),
	...
	data.ID = types.StringValue("discover_sliders")
